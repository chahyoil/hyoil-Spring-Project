				...
				<div id="notice-wrapper">
					<div class="notice-board">
						...
						<table class="board-table-content">
							<tbody>
								<c:forEach items="${member_ReviewList }" var="member_ReviewList">
									<tr onmouseover="this.style.background='#F7F7F7'"
										onmouseout="this.style.background=''">
										<td>${member_ReviewList.rbno }</td>
										<td><a
											href="member_ReviewContentView.do?rbno=${member_ReviewList.rbno }">${member_ReviewList.rbtitle }</a></td>
										<td>${member_ReviewList.mid}</td>
										<td>${member_ReviewList.rbtime}</td>
										<td>${member_ReviewList.rbhit }</td>
									</tr>
								</c:forEach>
							</tbody>
						</table>
						...
						<p class="btnArea">
							<span class="btn_button" style="line-height: 30px; width: 130px"
								onclick="location.href='member_ReviewWriteView.do'"> 후기작성 </span>
						</p>
						<!-- btnArea -->
					</div>
					<!-- notice-board -->
					<div class="board_pg_area">
						<c:if test="${paging.startPage>paging.blockSize}">
							[ <a
								href="member_ReviewList.do?pageNum=${paging.endPage-paging.blockSize }"
						...
						<c:forEach var="i" begin="${paging.startPage}"
							...
							<c:if test="${paging.currentPage != i }">
								[ <a href="member_ReviewList.do?pageNum=${i }">${i }</a> ]
							</c:if>
						</c:forEach>
						<c:if test="${paging.endPage<paging.pageCnt }">
							<a
								href="member_ReviewList.do?pageNum=${paging.startPage+paging.blockSize }"
