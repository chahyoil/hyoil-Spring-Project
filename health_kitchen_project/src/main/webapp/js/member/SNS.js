(function () {
    ...
    Social.naverCallback = function () {
        ...
        var data = { Acstkn: token, Id: id, Name: name, Gender: gender, Email: email, Mem_Type: "JK", RE_URL: Social.DecodeUrl(re_url) };
        ...
        if (id) {
            if (typeof $config !== 'undefined' && $config.connect === true) {
                ...
                obj.load("/naver/isjoin", "GET", { id: id, email: email }).done(function (response) {
                    ...
                    var params = "?RE_URL=" + Social.DecodeUrl(re_url) + "&acstkn=" + token + "&key=" + email + "&snscode=NV&uid=" + id + "&name=" + name + "&gender=" + gender;
    ...
    Social.naverJoin = function (data, isIframe) {
        ...
        qstring += "&Name=" + encodeURIComponent(data.Name) + "&Gender=" + data.Gender + "&Email=" + data.Email + "&isIframe=" + isIframe;
    ...
    Social.naverLogin = function (id, token, reurl, email) {
        var reurl = Social.DecodeUrl(reurl);
        if (!id) {
    ...
    Social.googleJoin = function (data, isIframe) {
       ...
       // var reurl = Social.DecodeUrl(data.RE_URL);
        ...
        qstring += "&Name=" + encodeURIComponent(data.Name) + "&Gender=" + data.Gender + "&Email=" + data.Email;
                        ...
                        if (id) {
                            if (typeof $config !== 'undefined' && $config.connect === true) {
                                ...
                                Social.load("/google/isjoin", "GET", { id: id, email: email }).done(function (response) {
                                    var hostname = $(location).attr('hostname');
                                    var params = "?RE_URL=" + re_url + "&acstkn=" + token + "&key=" + email + "&snscode=GL&uid=" + id + "&name=" + name + "&gender=" + gender;
                                    if (response.return_code > 0) {
                                        Social.FacebookPixel('AddPaymentInfo', function () {
    ...
    Social.kakaoStart = function () {
        Kakao.init('570f835c538b328e7401aa1c452a6dc9');
        var obj = this;
    ...
    Social.kakaoJoin = function (data, isIframe) {
        ...
        qstring += "&Name=" + data.Name + "&Gender=" + data.Gender + "&Email=" + data.Email;
    ...
    Social.FacebookJoin = function (data) {
        ...
        var reurl = "/Text_User/Resume/Resume_Step1.asp";
        var qstring = Social.getPcDomain() + "/Join/M_FB_Regist?Acstkn=" + encodeURIComponent(data.Acstkn) + "&Re_Url=" + reurl;
    ...
    Social.load = function (req, method, data, type) {
        var obj = this;
        var option;
        if (req.toLowerCase().indexOf('isjoin') > -1 || req.toLowerCase().indexOf('login/getmemexists') > -1)
        {
            option = {
                type: method,
                url: req,
    ...
    Social.getPcServerName = function () {
