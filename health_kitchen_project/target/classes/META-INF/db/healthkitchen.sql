...
select m2.MLEVELDC from member m1, membergrade m2 where m1.mid='hyoil' and m1.MLEVELNO=m2.MLEVELNO;
 --------------------------------공지사항게시판----------------------------------
...
---------------------------------TEST------------------------------------------
select count(pid), mid from CURRENTPRODUCT group by mid;
 select * from product;
select * from currentproduct c1, currentproduct c2 where c1.pid=c2.pid;
select * from currentproduct;
select * from currentproduct where 21 in pid and 'hyoil' not in mid; --111
select unique(pid) from currentproduct where mid='hyoil' and pid=21;

select pid, count(pid) as count_pid from currentproduct where 
    mid in (select mid from currentproduct where 21 in pid and 'hyoil' not in mid)
    group by pid 
    order by 2 desc;
